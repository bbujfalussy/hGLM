...
#################################################
## filters the input spike train with an alpha kernel defined by the time constant tau: f(t) = t/tau^2 exp(-t\tau)
## the amplitude of the function is 1/tau, while its integral is constant 1
...
alpha.filt <- function(tau, spikes, delay.t=0, dt=0.5, grad.tau=F, grad.delay=F, run.for=NULL){
	if (is.na(tau))	{
		# print("Tau is NA")
		return (-spikes)
	}
	...
	# if (grad.tau) tf <- seq(0, 10 * tau, by=dt) else tf <- seq(0, 10 * tau, by=dt)
	tf <- seq(0, 10 * tau, by=dt)
	L <- length(tf)
	filt <- (tf-delay.t)/tau^2 * exp(-(tf-delay.t)/tau); filt[filt<0] <- 0
	# (tf-delay.t)/tau^2 * exp(-(tf-delay.t)/tau)
	# -2 * (tf-delay.t)/tau^3 * exp(-(tf-delay.t)/tau) + (tf-delay.t)/tau^2 * ((tf-delay.t)/tau^2)* exp(-(tf-delay.t)/tau)
	# filt * (-2/tau + (tf-delay.t)/tau^2)
	if (grad.tau == T) filt <- filt * ((tf-delay.t)/tau^2 - 2/tau) # only if we compute the gradient of the error wrt tau!
	if (grad.delay == T) { # only if we compute the gradient of the error wrt delay.t!
		f0 <- as.numeric(filt>0)
		filt <- f0 * ((tf-delay.t)/tau^3 * exp(-(tf-delay.t)/tau) - 1/tau^2 * exp(-(tf-delay.t)/tau))
	}
	if (is.null(run.for)) {
...
# spikes <- rep(0, 500); spikes[100] <- 1
# tau <- 30
# dt <- 0.5
# grad.tau <- F

# fs <- alpha.filt(tau, spikes, grad.tau=F, dt=1)
# plot(fs, t="l")
# lines(spikes, t="h", col=3); abline(h=exp(-1)/tau, col=2)
# sum(fs)


# source('~/Programs/hGLMs/Rlib/AlphaFiltAmpl.R', chdir = TRUE)
# fs.a <- alpha.filt.ampl(tau, spikes, grad.tau=F, dt=1)
# plot(fs.a, col=2, t="l")
# lines(spikes, t="h", col=3); abline(h=exp(-1), col=2)
# lines(tau * fs, col=1)
...
# fs <- alpha.filt(tau, spikes, grad.tau=F)
# plot(fs, t="l")
# lines(spikes, t="h", col=3)

# spikes <- rep(0, 200); spikes[c(12,55,90)] <- c(1,1.2, 1.5)
# fs <- alpha.filt(tau, spikes, grad.tau=F, dt=5)
# lines(seq(10, 2000, by=10), fs, t="l", col=2)


# fs <- alpha.filt(tau, spikes, delay.t=10, grad.tau=F, dt=5)
# lines(seq(10, 2000, by=10), fs, t="l", col=2)
# abline(h=exp(-1)/tau, col=4, lty=2)

# spikes <- rep(0, 200); spikes[10] <- 1
# fs <- alpha.filt(tau, spikes, grad.tau=F)
# plot(fs, t="l", ylim=c(-0.01, 0.04))
# lines(spikes, t="h", col=3)
# fs <- alpha.filt(tau+1, spikes, grad.tau=F)
# lines(fs, t="l", col=3)
# fs <- alpha.filt(tau, spikes, delay.t=0, grad.tau=T)
# lines(fs, t="l", col=2)

# fs <- alpha.filt(tau, spikes, grad.delay=T)
# lines(fs, t="l", col=4)
...
# resp <- alpha.filt(tau=pars[1], spikes, delay.t=pars[2], dt=dt)
# args <- list(dt=dt, resp=resp, spikes=spikes)

# err.par <- function(pars, args){
	# resp <- alpha.filt(tau=pars[1], args$spikes, delay.t=pars[2], dt=args$dt)
...
# grad.par <- function(pars, args){
	# resp <- alpha.filt(tau=pars[1], args$spikes, delay.t=pars[2], dt=args$dt)
	# g.tau <- alpha.filt(tau=pars[1], args$spikes, delay.t=pars[2], dt=args$dt, grad.tau=T)
	# g.del <- alpha.filt(tau=pars[1], args$spikes, delay.t=pars[2], dt=args$dt, grad.delay=T)
...
# grad.tau <- F
# fs <- alpha.filt(2, spikes, grad.tau=F)
...
# for (tau in c(2, 4, 10, 20, 50)){
	...
	# ampl <- 1 / tau / (exp(-tpeak / (tau1 + dtau)) - exp(-tpeak / (tau1)))
